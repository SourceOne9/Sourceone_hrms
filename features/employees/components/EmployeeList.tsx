...
export const EmployeeList = React.memo(function EmployeeList({
    ...
    const columns = React.useMemo<ColumnDef<TableEmployee>[]>(() => [
        ...
        },
        {
            ...
            header: "Onboarding",
            cell: ({ row }) => {
                const ob = row.original.raw?.onboardingStatus || "pending"
                const variant = ob === "completed" ? "success" : ob === "in_progress" ? "warning" : "neutral"
                const label = ob === "completed" ? "Done" : ob === "in_progress" ? "In Progress" : "Pending"
                return <Badge variant={variant as any} size="sm" dot>{label}</Badge>
            },
        },
        {
            accessorKey: "start",
    ...
    return (
        <DataTable
            columns={columns}
            data={employees}
            onRowClick={(row: TableEmployee) => onOpenViewModal(row.raw)}
            searchKey="name"
