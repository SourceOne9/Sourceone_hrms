...
export function renderDailyReportEmail(data: DailyReportData): string {
    ...
    const appRows = data.topApps
        ...
        .map(a => {
            ...
            return `<tr><td style="padding:6px 8px;border-bottom:1px solid #f3f4f6">${a.appName}</td><td style="padding:6px 8px;border-bottom:1px solid #f3f4f6;text-align:right">${hrs}h</td><td style="padding:6px 8px;border-bottom:1px solid #f3f4f6">${a.category}</td></tr>`
        })
        .join("")
    ...
    const siteRows = data.topWebsites
        ...
        .map(w => {
            ...
            return `<tr><td style="padding:6px 8px;border-bottom:1px solid #f3f4f6">${w.domain}</td><td style="padding:6px 8px;border-bottom:1px solid #f3f4f6;text-align:right">${hrs}h</td><td style="padding:6px 8px;border-bottom:1px solid #f3f4f6">${w.category}</td></tr>`
        })
        .join("")

    const idleSection = data.idleNotes.length > 0
        ? `<div style="margin-top:24px"><h3 style="font-size:15px;margin:0 0 8px">Idle Event Notes</h3><ul style="margin:0;padding-left:20px;color:#6b7280">${data.idleNotes.map(n => `<li style="margin-bottom:4px">${n}</li>`).join("")}</ul></div>`
        : ""

    const recommendations = data.aiRecommendations.length > 0
        ? `<ul style="margin:8px 0 0;padding-left:20px;color:#374151">${data.aiRecommendations.map(r => `<li style="margin-bottom:4px">${r}</li>`).join("")}</ul>`
        : ""
...
<p style="margin:4px 0 0;font-size:14px;color:rgba(255,255,255,0.8)">${data.employeeName} — ${data.date}</p>
</div>
...
${idleSection}
${data.aiSummary ? `<div style="margin-top:24px;padding:16px;background:#f0f9ff;border-radius:8px;border-left:3px solid #3b82f6"><h3 style="font-size:15px;margin:0 0 8px;color:#1e40af">AI Insights</h3><p style="margin:0;font-size:13px;color:#374151;line-height:1.5">${data.aiSummary}</p>${recommendations}</div>` : ""}
