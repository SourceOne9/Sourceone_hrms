...
const resolveHierarchySchema = z.object({
...
async function handlePOST(req: Request) {
    try {
        ...
        const parsed = resolveHierarchySchema.safeParse(body)
        if (!parsed.success) {
        ...
        for (const row of rows) {
            ...
            if ((row as unknown as Record<string, unknown>).employeeCode) {
                csvRefs.set(normalizeKey((row as unknown as Record<string, unknown>).employeeCode as string), row)
            }
