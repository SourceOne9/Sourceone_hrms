...
export const GET = withAuth({ module: Module.PAYROLL, action: Action.VIEW }, async (req, ctx) => {
    try {
        ...
        const orgName = escapeHtml(payroll.organization.name)
        const month = escapeHtml(payroll.month)
        const firstName = escapeHtml(payroll.employee.firstName)
        const lastName = escapeHtml(payroll.employee.lastName)
        const empCode = escapeHtml(payroll.employee.employeeCode)
        const designation = escapeHtml(payroll.employee.designation)
        const status = escapeHtml(payroll.status)
        const payrollId = escapeHtml(payroll.id)
        ...
        <body>
            ...
            <div class="payslip-container">
                <div class="header">
                    <h1>${orgName}</h1>
                    <div class="subtitle">Payslip for the month of <strong>${month}</strong></div>
                ...
                <div class="section details-grid">
                    <div>
                        <div><strong>Employee Name:</strong> ${firstName} ${lastName}</div>
                ...
                <div style="margin-top: 50px; font-size: 11px; color: #94a3b8; text-align: center; border-top: 1px dashed #e2e8f0; padding-top: 20px;">
                    ...
                    Generated on ${new Date().toLocaleString()} | ID: ${payrollId}
                </div>
            </div>
        </body>
        </html>
